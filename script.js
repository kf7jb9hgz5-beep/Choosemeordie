...
function renderCanvas() {
    ...
    lines.forEach((l) => {
        ...
        content.innerHTML = `<div class="speaker-name" style="color:${s.nameColor}; font-size:${Math.max(11, fontSize - 2)}px">${s.name}</div>
                             <div class="dialogue-text" style="font-size:${fontSize}px; line-height:${lineHeight}px">${l.text}</div>`;
...
["bgColor", "padding", "fontSelect", "fontSize", "lineGap"].forEach(id => {
    ...
    if (el) { el.addEventListener("input", renderCanvas); el.addEventListener("change", renderCanvas); }
});
...
function captureCanvas(callback) {
    ...
    html2canvas(captureArea, { useCORS: true, allowTaint: true, backgroundColor: null, scale: 2 }).then(canvas => {
        captureArea.style.padding = originalPadding;
        callback(canvas);
    }).catch(() => { captureArea.style.padding = originalPadding; });
}
